# CUMCM Visual Skill｜数模国赛论文配图使用手册

> 面向数学建模竞赛、国赛论文、课程论文、毕业论文与答辩展示。  
> 当前推荐使用入口：Codex、Kimi K3。  
> 当前 GitHub 直装版为 v1.3.1，已通过 GitHub Pages 上线。

> 说明：本项目是面向 CUMCM 参赛学生的社区增强版，不是全国大学生数学建模竞赛组委会的官方工具。

## 一、它能做什么

CUMCM Visual Skill 是一套面向论文的可编辑可视化工具。它不只是制作完整 PPT，更主要的用途是把论文文字、模型公式和实验数据转换为可直接插入论文的图。它使用调用名 `$cumcm-visual-skill`，可与协会成员的 `$cumcm-figure-skill` 同时安装和使用。
...
名称迁移：如果电脑中仍安装着本项目上一版的 `$cumcm-figure-skill`，请先安装并确认新版 `$cumcm-visual-skill` 可用，再移除本项目旧副本。协会成员版继续使用 `$cumcm-figure-skill`，两套工具即可并存。
...
```text
请使用 cumcm-visual-skill，把附件中的研究内容生成一张思维导图。
...
```text
请使用 cumcm-visual-skill 读取附件中的 Excel。
...
```text
请使用 cumcm-visual-skill，根据附件中的模型 JSON 生成
...
```text
请使用 cumcm-visual-skill 读取附件中的 Markdown 论文。
...
```text
请使用 cumcm-visual-skill，根据【输入材料】生成一张【图型】。
