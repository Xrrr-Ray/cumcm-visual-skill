# CUMCM Visual Skill 学生使用与交付说明

> 面向数学建模竞赛、国赛论文、课程论文、毕业论文和答辩展示。  
> 当前推荐入口：Codex 与 Kimi K3。  
> 文档对应版本：v1.3.1（GitHub 直装版）。

## 1. 这是什么

CUMCM Visual Skill 不是一个“只能制作完整 PPT”的工具。它使用 `$cumcm-visual-skill` 调用名，可与协会成员的 `$cumcm-figure-skill` 同时安装。
...
```text
请使用 cumcm-visual-skill，把下面的论文结构生成一张思维导图。
...
```text
请使用 cumcm-visual-skill 读取这篇 Markdown 论文。
...
```text
请使用 cumcm-visual-skill，根据论文生成一份 10 页答辩 HTML PPT。
...
> CUMCM Visual Skill 是一个面向论文和数学建模竞赛的可编辑可视化系统。它支持 Codex 与 Kimi K3 理解论文内容，并使用统一生成器输出流程图、思维导图、模型关系图、数据图表、全文插图套件和多页 HTML 演示。生成结果可在浏览器中拖动编辑，也可导出 SVG、PNG 和 PowerPoint 可编辑 PPTX。
